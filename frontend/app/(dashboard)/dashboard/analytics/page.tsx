...
export default function AnalyticsPage() {
  ...
  return (
    <div className="space-y-8">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-bold text-[#1A1035]">Analytics</h1>
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        {[
          ...
          <div key={label} className="rounded-2xl border border-[#ede9f8] bg-white p-5 shadow-sm">
            ...
            <p className="mt-0.5 text-sm font-bold text-[#6B6490]">{label}</p>
            <p className="mt-0.5 text-xs text-[#6B6490]/70">{sub}</p>
          </div>
        ))}
      </div>
      ...
      {/* Per-bot breakdown */}
      <div>
        <h2 className="mb-4 text-base font-bold text-[#1A1035]">Bot Breakdown</h2>
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-[#ede9f8] py-20 text-center">
            ...
            <p className="mt-1 text-xs text-[#6B6490]">Create a bot to start seeing analytics.</p>
          </div>
        ) : (
          <div className="space-y-3">
            {data.map(({ bot, summary }) => (
              <div
                key={bot.id}
                className="flex items-center gap-4 rounded-2xl border border-[#ede9f8] bg-white p-5 shadow-sm"
              >
                {/* Avatar */}
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-2 flex flex-wrap gap-6">
                    {[
                      ...
                      <div key={label}>
                        <p className="text-xs text-[#6B6490]">{label}</p>
                        <p className="text-sm font-bold text-[#1A1035]">{value}</p>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div className="flex shrink-0 flex-col gap-2 sm:flex-row">
                  ...
                  <Link
                    ...
                    className="inline-flex items-center gap-1.5 rounded-xl border border-[#ede9f8] px-3 py-1.5 text-xs font-semibold text-[#6B6490] transition-colors hover:bg-[#faf8ff]"
                  >
