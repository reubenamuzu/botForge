...
export default function PricingSection() {
  return (
    <section id="pricing" className="relative overflow-hidden bg-[#faf8ff] py-28 border-t border-white/50">
      ...
      <div className="relative mx-auto max-w-6xl px-6">
        <div className="mb-20 text-center reveal">
          ...
          <p className="mt-4 text-lg text-[#6B6490] font-medium">Start for free. Upgrade as your customer base expands.</p>
        </div>
            ...
            <div
              ...
              className={`reveal delay-${i * 100} relative flex flex-col rounded-[2rem] p-6 transition-transform hover:-translate-y-1 duration-300 ${
                plan.highlight
                  ...
                  : 'bg-white/60 backdrop-blur-lg border border-white shadow-[0_8px_30px_rgba(0,0,0,0.04)]'
              }`}
            >
              {plan.highlight && (
              ...
              <div className="mb-8 flex items-baseline gap-1.5">
                <span className={`text-4xl font-extrabold tracking-tight ${plan.highlight ? 'text-white' : 'text-[#1A1035]'}`}>
                  {plan.price}
                </span>
                <span className={`text-sm font-semibold ${plan.highlight ? 'text-[#dabdff]' : 'text-[#6B6490]'}`}>
                  {plan.priceNote}
                </span>
              </div>
              <ul className="flex-1 space-y-4 mb-8">
                {plan.features.map((f) => (
                  <li key={f} className={`flex items-start gap-3 text-[14px] font-medium leading-snug ${plan.highlight ? 'text-white' : 'text-[#514972]'}`}>
                    <div className={`mt-0.5 rounded-full p-0.5 w-4 h-4 flex items-center justify-center shrink-0 ${plan.highlight ? 'bg-white/20' : 'bg-[#f4efff]'}`}>
              ...
              <Link
                ...
                className={`w-full rounded-2xl py-3.5 text-center text-[15px] font-bold transition-all ${
                  plan.highlight
                    ? 'bg-white text-[#6C47FF] hover:bg-[#f4efff] shadow-lg'
                    : 'bg-[#faf8ff] border border-[#e2d5fa] text-[#1A1035] hover:bg-white hover:border-[#cbc0e8] hover:shadow-md'
                }`}
              >
