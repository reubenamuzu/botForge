...
export default function FeaturesSection() {
  return (
    <section id="features" className="relative overflow-hidden bg-[#faf8ff] py-24">
      ...
      <div className="relative mx-auto max-w-6xl px-6">
        <div className="mb-16 text-center reveal">
          ...
          <p className="mt-4 text-lg text-[#6B6490] font-medium">
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {FEATURES.map(({ icon: Icon, title, desc, color }, i) => (
            <div
              key={title}
              className={`reveal delay-${Math.min(i * 100, 500)} rounded-3xl border border-white bg-white/50 backdrop-blur-xl p-8 shadow-[0_8px_30px_rgba(0,0,0,0.04)] hover:shadow-[0_12px_40px_rgba(100,50,200,0.08)] hover:-translate-y-1 transition-all duration-300`}
            >
              ...
              <h3 className="mb-3 text-lg font-bold text-[#1A1035]">{title}</h3>
              <p className="text-[15px] leading-relaxed font-medium text-[#6B6490]">{desc}</p>
            </div>
          ))}
        </div>
      </div>
