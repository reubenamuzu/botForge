...
function PhoneFrame({ children, className = '' }: { children: ReactNode; className?: string }) {
  return (
    <div className={`relative rounded-[2.8rem] border-[7px] border-[#1C1C1E] bg-[#1C1C1E] shadow-2xl shadow-black/25 ${className}`}>
      ...
      {/* Screen */}
      <div className="rounded-[2.2rem] overflow-hidden relative bg-white h-full">
        {/* Dynamic island */}
...
function TwoPhoneMockup() {
  return (
    <div className="relative mx-auto flex items-end justify-center w-full max-w-4xl h-[560px] md:h-[650px] mt-2">
      ...
      <div className="absolute top-[20%] left-[5%] md:left-[10%] z-30 flex animate-bounce [animation-duration:4s] items-center gap-3 rounded-2xl border border-white/60 bg-white/60 backdrop-blur-md px-4 py-3 shadow-[0_8px_32px_rgba(0,0,0,0.05)]">
        <div className="flex h-10 w-10 items-center justify-center rounded-full bg-[#f4ebff] shadow-sm border border-white/40">
          <Mic className="h-5 w-5 text-[#9a70ff]" />
        </div>
        <div className="flex flex-col text-left">
          <span className="text-sm font-bold text-[#1A1035]">Answers on the go.</span>
          <span className="text-xs font-semibold text-[#6B6490]">Just speak up.</span>
        </div>
      </div>

      {/* Floating card — bottom left */}
      <div className="absolute bottom-[20%] left-[5%] md:left-[8%] z-30 flex animate-bounce [animation-duration:5s] items-center gap-4 rounded-[2rem] border border-white/60 bg-white/70 backdrop-blur-xl px-5 py-3 shadow-[0_8px_32px_rgba(100,50,250,0.1)]">
      ...
      <div className="absolute top-[35%] right-[2%] md:right-[5%] z-30 flex animate-bounce [animation-duration:4.5s] items-center gap-3 rounded-full border border-white/60 bg-[#eedcff]/60 backdrop-blur-md pl-2 pr-6 py-2 shadow-xl shadow-purple-200/40">
      ...
      <div className="absolute bottom-[25%] right-[5%] md:right-[12%] z-30 flex animate-bounce [animation-duration:3.5s] flex-col items-center justify-center rounded-2xl border border-white/60 bg-white/60 backdrop-blur-md p-4 shadow-[0_12px_24px_rgba(150,100,250,0.15)]">
        <div className="flex h-10 w-10 items-center justify-center rounded-full bg-white shadow-sm mb-3">
          <ChevronsUp className="h-5 w-5 text-[#1A1035]" />
        </div>
        <span className="text-xs font-bold text-[#1A1035] text-center">High-speed<br />processing</span>
      </div>
      ...
      <div className="absolute right-[15%] md:right-[25%] bottom-10 z-10 w-[220px] md:w-[260px] h-[450px] md:h-[530px]">
        <PhoneFrame className="w-full h-full shadow-2xl scale-95 origin-bottom">
          <div className="flex flex-col bg-white h-full">
            <div className="border-b border-[#F5F3FF] bg-white px-4 pt-12 pb-4 shrink-0">
              <p className="text-xs font-bold text-[#1A1035]">BotForge Dashboard</p>
            ...
            <div className="p-3 space-y-3 flex-1 overflow-hidden">
              <div className="grid grid-cols-2 gap-2">
                <div className="rounded-xl bg-[#F5F3FF] p-2.5">
                  <p className="text-[10px] text-[#6B6490]">Messages</p>
                  <p className="text-lg font-extrabold text-[#6C47FF]">3.4k</p>
                </div>
                <div className="rounded-xl bg-emerald-50 p-2.5">
                  <p className="text-[10px] text-[#6B6490]">Resolved</p>
                  <p className="text-lg font-extrabold text-emerald-600">91%</p>
                </div>
              </div>
              <div className="rounded-xl bg-[#F5F3FF] p-3">
                <p className="text-[10px] font-semibold text-[#6B6490] mb-2">Recent conversations</p>
                {['Shipping to Kumasi', 'Return policy', 'Size guide'].map((item) => (
                  <div key={item} className="flex items-center justify-between py-1.5 border-b border-[#EDE8FF] last:border-0">
                    <p className="text-[10px] text-[#1A1035]">{item}</p>
      ...
      <div className="absolute left-[15%] md:left-[25%] bottom-0 z-20 w-[230px] md:w-[280px] h-[470px] md:h-[560px]">
        <PhoneFrame className="w-full h-full shadow-[-20px_0_60px_rgba(100,50,200,0.25)]">
          <div className="flex flex-col bg-[#F5F3FF] h-full">
            <div className="flex items-center gap-2.5 bg-[#6C47FF] px-4 pt-12 pb-4 shrink-0">
              <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-white/20 text-xs font-bold text-white">SK</div>
              <div>
            ...
            <div className="border-t border-[#EDE8FF] bg-white p-3 shrink-0">
...
export default function HeroSection() {
  return (
    <section className="relative overflow-hidden bg-[#faf8ff] pb-10 min-h-screen flex items-center pt-20 md:pt-10">
      {/* Aurora cloud background */}
      <div className="absolute inset-0 z-0 overflow-hidden pointer-events-none">
        <div className="absolute top-[-10%] left-[-5%] h-[600px] w-[800px] rounded-[100%] bg-[#ecdfff] blur-[100px] mix-blend-multiply opacity-50 animate-aurora-1" />
        <div className="absolute top-[10%] right-[-10%] h-[800px] w-[800px] rounded-[100%] bg-[#ead6ff] blur-[120px] mix-blend-multiply opacity-60 animate-aurora-2" />
        <div className="absolute top-[30%] left-[20%] h-[600px] w-[800px] rounded-[100%] bg-[#fff0f8] blur-[100px] mix-blend-multiply opacity-60 animate-aurora-3" />
        <div className="absolute bottom-[-10%] left-[0%] h-[700px] w-[900px] rounded-[100%] bg-[#e3d6f5] blur-[100px] mix-blend-multiply opacity-40 animate-aurora-4" />
      </div>
      ...
      <div className="relative mx-auto flex w-full max-w-5xl flex-col items-center px-6 text-center z-10 pt-16">
        ...
        {/* Headline */}
        <h1
          className="hero-in max-w-4xl text-2xl font-bold leading-[1.2] tracking-tight text-[#1A1035] sm:text-3xl lg:text-[4rem] lg:leading-[1.15]"
        ...
        {/* Subtitle */}
        <p
          className="hero-in mt-6 max-w-[500px] text-base font-normal leading-relaxed text-[#6B6490]"
        ...
        >
          <Link
            href="/sign-up"
            className="flex items-center justify-center gap-2 rounded-full bg-gradient-to-r from-[#dcd1ff] to-[#ccb5ff] hover:from-[#d1c1fa] hover:to-[#be98fc] px-8 py-3.5 text-base font-semibold text-[#1A1035] shadow-lg shadow-purple-900/10 transition-all hover:scale-105"
          >
            Get started free <ArrowUpRight className="h-4 w-4" />
          </Link>
          <a
            href="#how-it-works"
            className="flex items-center justify-center gap-2 rounded-full border border-[#f0eaff] bg-white/70 backdrop-blur-md px-8 py-3.5 text-base font-semibold text-[#1A1035] shadow-sm hover:bg-white transition-all hover:border-[#dfcfff]"
          >
            Learn More
          </a>
        </div>
